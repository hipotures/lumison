...
export function createFilmMaterial(qualityName) {
  ...
  const graph = Fn(() => {
    ...
    const pdd = pd.x.mul(pd.x).add(pd.y.mul(U.uAspect).mul(pd.y.mul(U.uAspect)));
    const push = exp(pdd.div(-0.035)).mul(U.uPointer.z);
    const advPushed = pack0.adv.add(U.uPointerVel.mul(push.mul(1.05)));
    ...
    const dragShear = pd.dot(U.uPointerVel).mul(exp(pdd.div(-0.045))).mul(U.uPointer.z.mul(-0.20));
