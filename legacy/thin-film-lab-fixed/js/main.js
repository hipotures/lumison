...
async function measurePresentationHz(sampleCount = 24) {
  // requestAnimationFrame is synchronized to the display compositor. Measure
  // that presentation ceiling before the expensive renderer starts, so a
  // user-selected 120 fps target on a 60 Hz display does not make adaptive
  // quality chase an impossible visible frame rate.
  ...
  return await new Promise((resolve) => {
    ...
    const tick = (t) => {
      ...
      if (deltas.length >= sampleCount) {
        ...
        for (const hz of standards) if (Math.abs(hz - raw) < Math.abs(nearest - raw)) nearest = hz;
...
async function boot() {
  ...
  function frame() {
    ...
    if (app.renderer && state.adaptive) {
      const action = adaptiveTick(app.adaptive, app.perf.fps, now, {
        enabled: true,
        // Visible canvas presentation is rAF/VSync limited. Keep the user's
        // requested value, but do not punish quality for refreshes the display
        // physically cannot present.
        targetFps: Math.min(state.targetFps, app.displayHz),
  ...
  function updateDiag() {
    ...
    const lines = [
      ...
      `quality  <b>${app.renderer ? app.effQuality : '—'}</b>${state.adaptive ? ` (auto scale only · ${app.adaptive.lastAction})` : ' (manual)'}`,
